...
import Data.Array (filter, intersect, null, uncons)
...
main :: forall e. Eff (exception :: EXCEPTION, console :: CONSOLE, testOutput :: TESTOUTPUT, avar :: AVAR, fs :: F.FS | e)
...
loadSpecs path =
  ...
  where
    ...
    filterDocs :: Array SampleDoc -> Array SampleDoc
    filterDocs = filter (\d -> null $ intersect d.tags skipSpecsTagged)

    assertions :: forall e. Pattern -> Schema -> Array SampleDoc -> Aff e Unit
    assertions pattern schema docs = case uncons $ (assertion pattern schema) <$> docs of
      Just { head, tail } -> foldl bind head (const <$> tail)
    ...
    assertion pattern schema doc =
      ...
      where
        actualErrors :: Array ValidationError
        actualErrors = case validate schema doc.json pattern of
          Left errs -> errs
          Right _ -> mempty

        actualErrorPaths :: Array JsonPath
        actualErrorPaths = (\(ValidationError err) -> err.path) <$> actualErrors

        unexpected :: Array ValidationError
        unexpected = filter (\(ValidationError err) -> not $ elem err.path doc.expectedErrors) actualErrors

        missing :: Array JsonPath
        missing = filter (\i -> not $ elem i actualErrorPaths) doc.expectedErrors
